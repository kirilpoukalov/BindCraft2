...
| `Nanobody` | `scaffolds/Nanobody.cif` | Chain A of **PDB 3EAK** (NbBCII10-FGLA, *Camelus dromedarius*), 127 residues, waters and the unresolved C-terminal His-tag dropped and residues numbered 1–127. A genuine **camelid** VHH framework with its native 18-residue CDR3, which in the crystal lies folded back across the framework (20 framework partners within 5 Å, against 18 for `VHH.cif`). Paratope: CDR1 26–36, CDR2 54–61, CDR3 100–117. Two framework rows, `A50*` and `A62*`, are declared so `binder_intra_coldspot` has a face to steer against and both CDR3 conformations can be sampled. They were chosen by the loss's own criterion — CB–CB under 8 Å with sequence separation of at least 20 — as the positions carrying real CDR3 packing while staying solvent-accessible (A62 four contacts, closest 4.10 Å, 42.6 Å²; A50 four contacts, 5.27 Å, 21.4 Å²). The FR2 rows the shipped `VHH` preset opens are unsuitable here: A38 and A36 sit at 0.0 Å² SASA, A40 at 2.4 and A53 at 0.7, so opening them would put buried core up for redesign, and A48, despite 79 Å² exposure, has only one eligible contact — a dead row that dilutes the loss, which averages over every declared framework row. Because declaring framework rows suppresses the automatic `designed_only=False` for `binder_coldspot`, the preset sets it explicitly so target repulsion still covers the whole framework rather than just those two rows. Unlike `VHH` this is **not** a human germline scaffold. |
